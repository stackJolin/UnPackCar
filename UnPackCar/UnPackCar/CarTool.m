...
void exportCarFileAtPath(NSString * carPath, NSString *outputDirectoryPath, void(^callback)(CGFloat))
{
    NSError *error = nil;
    ...
    NSInteger count = [storage allRenditionNames].count;
    NSInteger current = 0;
    for (NSString *key in [storage allRenditionNames])
    {
        ...
        current += 1;
        if (callback) {
            callback((CGFloat)current / (CGFloat)count);
        }
    }
}
